...
import 'get_all_shared_space_members_interactor_test.mocks.dart';
...
void main() {
  group('get_all_shared_space_members_interactor', () {
    ...
    setUp(() {
      ...
      _getAllSharedSpaceMembersInteractor = GetAllSharedSpaceMembersInteractor(sharedSpaceMemberRepository);
    });
    ...
    test('getAllSharedSpaceMembersInteractor should return success with members list', () async {
      ...
      final membersList = result.map((success) => (success as SharedSpaceMembersViewState).members)
          .getOrElse(() => []);

      expect(membersList, containsAllInOrder([sharedSpaceMember1, sharedSpaceMember2]));
    });
    ...
    test('getAllSharedSpaceMembersInteractor should fail when getMembers fail', () async {
      ...
      result.fold(
        (failure) => expect(failure, isA<SharedSpaceMembersFailure>()),
        (success) => expect(success, isA<SharedSpaceMembersViewState>()));

      expect(result, Left<Failure, Success>(SharedSpaceMembersFailure(exception)));
    });
  });
}
