...
void main() {
  group('test shared space activities interactor', () {
    ...
    setUp(() {
      ...
      _sharedSpaceActivitiesInteractor = SharedSpaceActivitiesInteractor(_sharedSpaceActivitiesRepository);
    });
    ...
    test('SharedSpaceActivitiesInteractor should return success with valid data', () async {
      ...
      final activities = result
          .map((success) => (success as SharedSpacesActivitiesViewState).auditLogEntryUserList)
          .getOrElse(() => []);
      expect(activities, [documentAuditLog, folderAuditLog]);
    });
    ...
    test('SharedSpaceActivitiesInteractor should fail when getSharedSpaceActivities fail', () async {
      ...
      final result = await _sharedSpaceActivitiesInteractor.execute(sharedSpaceIdForAuditLog);
      expect(result, Left<Failure, Success>(SharedSpacesActivitiesFailure(exception)));
    });
  });
}