...
void main() {
  group('remove_received_share_interactor_test', () {
    ...
    test('remove received share should return success with valid data', () async {
      ...
      final result = await removeReceivedShareInteractor.execute(receivedShare1.shareId);
      result.fold(
        (failure) => null,
        (success) {
          expect(success, isA<RemoveReceivedShareViewState>());
          expect(receivedShare1, (success as RemoveReceivedShareViewState).receivedShare);
      });
    });

    test('remove received share should return failure', () async {
      when(receivedShareRepository.remove(receivedShare1.shareId))
        .thenThrow(Exception());
      final result = await removeReceivedShareInteractor.execute(receivedShare1.shareId);
      result.fold(
