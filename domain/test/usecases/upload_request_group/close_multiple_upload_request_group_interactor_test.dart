...
import 'close_multiple_upload_request_group_interactor_test.mocks.dart';

@GenerateMocks([UploadRequestGroupRepository])
void main() {
  group('close_multiple_upload_request_group_interactor tests', () {
    ...
    test('close multiple upload_request_group should return success with valid data', () async {
      ...
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup2, UploadRequestStatus.CLOSED))
          .thenAnswer((_) async => uploadRequestGroupClosed2);

      final result = await multipleUploadRequestGroupStateInteractor.execute(
          [uploadRequestGroup1, uploadRequestGroup2], UploadRequestStatus.CLOSED);
      ...
      (state as UpdateUploadRequestGroupAllSuccessViewState).resultList[0].fold(
        (failure) => {},
        (success) => expect((success as UpdateUploadRequestGroupStateViewState).uploadRequestGroup, uploadRequestGroupClosed1));

      state.resultList[1].fold(
        (failure) => {},
        (success) => expect((success as UpdateUploadRequestGroupStateViewState).uploadRequestGroup, uploadRequestGroupClosed2));
    });

    test('close multiple upload_request_group should return success with some failed groups to close', () async {
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup1, UploadRequestStatus.CLOSED))
          .thenAnswer((_) async => uploadRequestGroupClosed1);
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup2, UploadRequestStatus.CLOSED))
          .thenThrow(Exception());
    ...
    test('close multiple upload_request_group should return failure with all failed groups to close', () async {
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup1, UploadRequestStatus.CLOSED))
          .thenThrow(Exception());
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup2, UploadRequestStatus.CLOSED))
          .thenThrow(Exception());
