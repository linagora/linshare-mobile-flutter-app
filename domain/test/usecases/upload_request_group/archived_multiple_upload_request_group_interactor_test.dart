...
import 'archived_multiple_upload_request_group_interactor_test.mocks.dart';

@GenerateMocks([UploadRequestGroupRepository])
void main() {
  group('archive_multiple_upload_request_group_interactor tests', () {
    ...
    test('archive multiple upload_request_group should return success with valid data', () async {
      ...
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup2, UploadRequestStatus.ARCHIVED))
          .thenAnswer((_) async => uploadRequestGroupArchived2);

      final result = await multipleUploadRequestGroupStateInteractor.execute(
          [uploadRequestGroup1, uploadRequestGroup2], UploadRequestStatus.ARCHIVED);
      ...
      (state as UpdateUploadRequestGroupAllSuccessViewState).resultList[0].fold(
        (failure) => {},
        (success) => expect((success as UpdateUploadRequestGroupStateViewState).uploadRequestGroup, uploadRequestGroupArchived1));

      state.resultList[1].fold(
        (failure) => {},
        (success) => expect((success as UpdateUploadRequestGroupStateViewState).uploadRequestGroup, uploadRequestGroupArchived2));
    });

    test('archive multiple upload_request_group should return success with some failed groups to archive', () async {
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup1, UploadRequestStatus.ARCHIVED))
          .thenAnswer((_) async => uploadRequestGroupArchived1);
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup2, UploadRequestStatus.ARCHIVED))
          .thenThrow(Exception());
    ...
    test('archive multiple upload_request_group should return failure with all failed groups to archive', () async {
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup1, UploadRequestStatus.ARCHIVED))
          .thenThrow(Exception());
      when(uploadRequestGroupRepository.updateUploadRequestGroupState(uploadRequestGroup2, UploadRequestStatus.ARCHIVED))
          .thenThrow(Exception());
