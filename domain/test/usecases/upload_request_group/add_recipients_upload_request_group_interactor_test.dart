...
void main() {
  group('add_recipients_upload_request_group_interactor test', () {
    ...
    late AddRecipientsToUploadRequestGroupInteractor addRecipientsToUploadRequestGroupInteractor;

    setUp(() {
      uploadRequestGroupRepository = MockUploadRequestGroupRepository();
      addRecipientsToUploadRequestGroupInteractor = AddRecipientsToUploadRequestGroupInteractor(uploadRequestGroupRepository);
    });

    test('addRecipientsToUploadRequestGroupInteractor should return success with one valid data', () async {
      when(uploadRequestGroupRepository.addRecipients(
        uploadRequestGroup1.uploadRequestGroupId,
        [genericUser1, genericUser])
      ).thenAnswer((_) async => uploadRequestGroup1);

      final result = await addRecipientsToUploadRequestGroupInteractor.execute(
        uploadRequestGroup1.uploadRequestGroupId,
        [genericUser1, genericUser]
      );
    ...
    test('addRecipientsToUploadRequestGroupInteractor should fail when addRecipients fail', () async {
      ...
      expect(result, Left<Failure, Success>(AddRecipientsToUploadRequestGroupFailure(exception)));
    });
  });
}