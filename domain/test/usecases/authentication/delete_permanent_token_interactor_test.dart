...
void main() {
  ...
  group('delete_permanent_token_interactor_test', () {
    ...
    test('deletePermanentTokenInteractor should fail with wrong token', () async {
      when(tokenRepository.getToken()).thenAnswer((_) async => wrongToken);
      when(authenticationRepository.deletePermanentToken(wrongToken))
          .thenThrow(RequestedTokenNotFound());
      final result = await deletePermanentTokenInteractor.execute();
      expect(result, Left<Failure, Success>(LogoutFailure(RequestedTokenNotFound())));
    });

    test('deletePermanentTokenInteractor should fail with connection error', () async {
      when(tokenRepository.getToken()).thenAnswer((_) async => token);
      when(authenticationRepository.deletePermanentToken(token))
          .thenThrow(ConnectError());
      final result = await deletePermanentTokenInteractor.execute();
      expect(result, Left<Failure, Success>(LogoutFailure(ConnectError())));
    });

    test('deletePermanentTokenInteractor should fail with unknown error', () async {
      when(tokenRepository.getToken()).thenAnswer((_) async => token);
      when(authenticationRepository.deletePermanentToken(token))
          .thenThrow(UnknownError('unknown error'));
      final result = await deletePermanentTokenInteractor.execute();
      expect(result, Left<Failure, Success>(LogoutFailure(UnknownError('Unknown Error'))));
    });
  });
}
