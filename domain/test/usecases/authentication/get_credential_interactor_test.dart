...
void main() {
  ...
  group('get_credential_interactor_test', () {
    ...
    test('getCredentialInteractor should failure with no BaseUrl', () async {
      ...
      final result = await getCredentialInteractor.execute();
      expect(result, Left(CredentialFailure(BadCredentials())));
    });

    test('getCredentialInteractor should failure with empty baseUrl', () async {
      when(tokenRepository.getToken()).thenAnswer((_) async => permanentToken);
      when(credentialRepository.getBaseUrl()).thenAnswer((_) async => Uri.parse(''));
      final result = await getCredentialInteractor.execute();
      expect(result, Left(CredentialFailure(BadCredentials())));
    });

    test('getCredentialInteractor should failure with no token', () async {
