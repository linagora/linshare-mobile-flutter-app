  ...
  Future<Either<Failure, Success>> execute({required List<Document> documents}) async {
    ...
    if (listResult.length == 1) {
      ...
      if (failedFileCount == 0) {
        return Right(DuplicateMultipleToMySpaceAllSuccessViewState(listResult));
      } else if (failedFileCount == listResult.length) {
        return Left(DuplicateMultipleToMySpaceAllFailure(listResult));
      }
      return Right(DuplicateMultipleToMySpaceHasSomeFilesViewState(listResult));
    }
  }
}
