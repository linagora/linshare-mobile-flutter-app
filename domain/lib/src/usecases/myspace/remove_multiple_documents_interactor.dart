  ...
  Future<Either<Failure, Success>> execute({@required List<DocumentId> documentIds}) async {
    ...
    if (listResult.length == 1) {
      ...
      if (failedFileCount == 0) {
        return Right(RemoveMultipleDocumentsAllSuccessViewState(listResult));
      } else if (failedFileCount == listResult.length) {
        return Left(RemoveMultipleDocumentsAllFailureViewState(listResult));
      }
      return Right(RemoveMultipleDocumentsHasSomeFilesFailedViewState(listResult));
    }
  }
}
