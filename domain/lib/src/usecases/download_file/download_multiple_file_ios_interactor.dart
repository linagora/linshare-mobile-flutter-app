  ...
  Future<Either<Failure, Success>> execute({@required List<Document> documents, @required CancelToken cancelToken}) async {
    ...
    if (listResult.length == 1) {
      ...
      if (failedFileCount == 0) {
        return Right(DownloadFileIOSAllSuccessViewState(listResult));
      } else if (failedFileCount == listResult.length) {
        return Left(DownloadFileIOSAllFailureViewState(listResult));
      }
      return Right(DownloadFileIOSHasSomeFilesFailureViewState(listResult));
    }
  }
}
