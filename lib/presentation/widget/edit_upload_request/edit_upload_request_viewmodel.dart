...
class EditUploadRequestViewModel extends BaseViewModel {
  ...
  void initState(EditUploadRequestArguments argument) {
    getUploadRequestGroup(argument.uploadRequestGroup.uploadRequestGroupId);
  }

  void getUploadRequestGroup(UploadRequestGroupId uploadRequestGroupId) {
    store.dispatch(OnlineThunkAction((Store<AppState> store) async {
      store.dispatch(StartEditUploadRequestLoadingAction());

      await _getUploadRequestGroupInteractor.execute(uploadRequestGroupId).then((result) =>
          result.fold((failure) {
            uploadRequestGroup = null;
            store.dispatch(EditUploadRequestSetUploadRequestGroupAction(Left(failure)));
          }, (success) {
            uploadRequestGroup = success is SingleUploadRequestGroupViewState ? success.uploadRequestGroup : null;
            store.dispatch(EditUploadRequestSetUploadRequestGroupAction(Right(success)));
          }));
    }));
  }

  void backToUploadRequestGroups() {
