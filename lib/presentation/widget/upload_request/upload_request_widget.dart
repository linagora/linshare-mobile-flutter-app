...
class UploadRequestWidget extends StatefulWidget {
  final UploadRequestGroup uploadRequestGroup;
  final OnBackUploadRequestClickedCallback onBackUploadRequestClickedCallback;

  UploadRequestWidget(Key key, this.uploadRequestGroup, {required this.onBackUploadRequestClickedCallback})
      : super(key: key);

  @override
  _UploadRequestWidgetState createState() => _UploadRequestWidgetState();
}

class _UploadRequestWidgetState extends State<UploadRequestWidget> {
  final _model = getIt<UploadRequestViewModel>();
  final imagePath = getIt<AppImagePaths>();

  @override
  void initState() {
    Future.delayed(Duration.zero, () {
      _model.initState(widget.uploadRequestGroup);
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      _buildTopBar(),
      StoreConnector<AppState, UploadRequestState>(
        converter: (store) => store.state.uploadRequestState,
        builder: (_, state) =>
            Expanded(child: _buildUploadRequestListWidget(context, state.uploadRequestsList)),
      )
    ]);
  }

  Widget _buildUploadRequestListWidget(
      BuildContext context, List<UploadRequest> uploadRequestList) {
    return RefreshIndicator(
        onRefresh: () async => _model.getUploadRequest(widget.uploadRequestGroup),
  ...
  ListTile _buildActiveClosedUploadTile(UploadRequest request) {
    return ListTile(
      dense: true,
      leading: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        SvgPicture.asset(imagePath.icCollective, width: 20, height: 24, fit: BoxFit.fill)
      ]),
        ...
        builder: (context, searchStatus) => searchStatus == SearchStatus.ACTIVE
            ...
            : Container(
                ...
                decoration: BoxDecoration(color: Colors.white, boxShadow: [
                  BoxShadow(
                      color: Colors.black.withOpacity(0.06), blurRadius: 20, offset: Offset(0, 4))
                ]),
                child: Row(children: [
                  GestureDetector(
  ...
  Widget _buildTitleTopBar() {
    return GestureDetector(
        ...
        child: Container(
          child: Align(
            alignment: Alignment.center,
            child: Text(AppLocalizations.of(context).back_to_upload_requests,
                style: TextStyle(
                    fontSize: 18,
