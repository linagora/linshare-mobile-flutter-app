...
class SharedSpaceDataSourceImpl implements SharedSpaceDataSource {
  ...
  Future<SharedSpaceNodeNested> createNewWorkSpace(CreateWorkSpaceRequest createWorkSpaceRequest) {
    return Future.sync(() async {
      return (await _linShareHttpClient.createNewWorkSpace(createWorkSpaceRequest.toCreateWorkSpaceBodyRequest()))
          .toSharedSpaceNodeNested();
    }).catchError((error) {
      _remoteExceptionThrower.throwRemoteException(error, handler: (DioError error) {
        if (error.response?.statusCode == 404) {
  ...
  Future<SharedSpaceNodeNested> renameWorkSpace(SharedSpaceId sharedSpaceId, RenameWorkSpaceRequest renameRequest) {
    return Future.sync(() async {
      return (await _linShareHttpClient.renameWorkSpace(sharedSpaceId, renameRequest.toRenameWorkSpaceBodyRequest()))
          .toSharedSpaceNodeNested();
    }).catchError((error) {
      _remoteExceptionThrower.throwRemoteException(error, handler: (DioError error) {
        if (error.response?.statusCode == 404) {
