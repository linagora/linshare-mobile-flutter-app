...
class LinShareHttpClient {
  ...
  AuditLogEntryUserDto? _convertToAuditLogEntryNodeChild(Map<String, dynamic> nodeChildJson) {
    ...
    } else if (nodeChildJson['type'] == AuditLogEntryType.UPLOAD_REQUEST.value) {
  ...
  Future<SharedSpaceMemberResponse> updateRoleWorkspaceMember(SharedSpaceId sharedSpaceId, UpdateWorkspaceMemberRequest request, {bool? isOverrideRoleForAll}) async {
    final resultJson = await _dioClient.put(
        Endpoint.sharedSpaces
            .withPathParameter(sharedSpaceId.uuid)
            .withPathParameter('members')
  ...
  Future<UploadRequestResponse> getUploadRequest(UploadRequestId uploadRequestId) async {
    final resultJson = await _dioClient.get(Endpoint.uploadRequestsRoute
        .withPathParameter(uploadRequestId.uuid)
        .generateEndpointPath());
    return UploadRequestResponse.fromJson(resultJson);
  }
  ...
  Future<List<AuditLogEntryUserDto?>> getUploadRequestActivities(UploadRequestId uploadRequestId) async {
    final membersJson = await _dioClient.get(Endpoint.uploadRequestsRoute
        .withPathParameter(uploadRequestId.uuid)
        .withPathParameter('audit')
        .generateEndpointPath());
    return membersJson
